...
| `src/lx521_baffle/obiwan/attachments.py` | Optional tweeter crescent with complete standalone blind-M3 receiver ears plus two mirrored vertical M2 x 8 center ties at x=+/-13. An M2 head is Ø3.8 x 2.0 in every drive style, so a bore that swallows one cannot be under Ø4.2; rather than show that on the scallop, the crescent takes the head sideways through a rear-facing Ø4.6 channel -- which clears the head pocket radially and overshoots its axis, both so the shank can drop past the overhang and so no face meets the pocket tangentially, and which stops 1.5 mm short of the mating face it exists to clamp -- while only a Ø2.4 hex-key passage continues to the scallop edge, leaving a single Ø2.4 mark with the head 8.3 mm inside. The UM owns the blind Ø3.2 heat-set receivers. The direction is forced by access: the UM's rear recess is filled here by the buried tweeter cover, so only the crescent has an open corridor. Both ties are gated by a screw/hex-key access probe and a receiver burial probe on the actual solids); any cable retention is external/non-modeled, and magnets receive zero structural load credit |
