...
Obi-Wan has no seam B, so the vase above cannot fit it. Its BMR option is a
crescent instead: `obiwan_bmr_crescent_TEBM35C10-4`, a candidate part built
by the local-only `make obiwan_bmr_crescent_cad` into
`build/bmr_crescent_TEBM35C10-4/`. It is grown from the released ND25FW-4
crescent, so its UM-collar mate is identical by construction and the two
crescents are swappable without touching the UM print. Full description,
including the six open qualification items, is in
