...
Compiled 2026-08-21 from the source constants (every bore cited to its
...
| 2 | **M3 brass heat-set** | 4.6 | 4.0 | Ø4.6 × 4.0 (+0.2 overshoot where blind) | `base.py` `UM_PILOT_*`; `carriers.py` `JOINT_INSERT_*`, `TWEETER_JOINT_INSERT_*`; `b2_split.py` `SEAM_B_M3_*`; `floor.py` `NL8_*` |
...
|------|-----------|--------|-------|-----------|
| TEBM35C10-4 mounts, PCD 48.26, 4 × 90° | BMR vases (1 driver) | M2 | 4 | `tebm35c10_4_land.py` `TEBM_MOUNT_*`, `M2_INSERT_*` |
...
| stock A/B1/B2 or slim V1-A/B1/V1L, bridge | 10 (6 LM + 4 bridge) | 5 (4 UM + 1 vase seam) | 0 (+4 if the BMR vase variant) |
