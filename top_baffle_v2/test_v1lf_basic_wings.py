"""Remote-only Ac/Ae V1LF wing acceptance checks.
...
NO_FLOOR_STAGE_MANIFEST = (
    ROOT / "no_floor_stand/.v1lf_stage/manifest.json")
FLOOR_STAGE_MANIFEST = ROOT / "floor_stand/.v1lf_stage/manifest.json"
VARIANT_IDS = ("ac", "ae")
...
def _artifact_root() -> Path:
    raw = os.environ.get("LX_V1LF_BASIC_WING_ARTIFACT_ROOT", "wings")
    path = Path(raw)
    if not path.is_absolute():
        path = ROOT / path
    path = path.resolve()
    assert path == (ROOT / "wings").resolve(), (
        "the Ac/Ae release contract is rooted at top_baffle_v2/wings")
    return path
...
def _variant_paths(slug: str) -> dict[str, Path | tuple[Path, ...]]:
    ...
    return {
        "directory": directory,
        "canonical_step": (
            directory / f"top_baffle_nd25fw4_v1lf_wing_{slug}.step"),
        "assembled_step": (
            directory
...
def _assert_review_png(path: Path, slug: str, kind: str) -> None:
    ...
    assert (isinstance(description, str)
            and "dual-state V1LF LM-lower silhouettes: coincident common "
                "profile, blue dash-dot no-floor and green dotted floor "
                "stand" in description), (
        f"{path}: missing V1LF context-silhouette metadata")
    required_phrase = {
...
def _source_hash_records(source: dict) -> tuple[tuple[str, str], ...]:
    ...
    assert module == "v1lf_basic_wings_cad", (
...
def test_exported_artifact_contract() -> None:
    ...
    for slug in VARIANT_IDS:
        ...
        canonical_labels = (
            f"lx521_v1lf_basic_wing_{slug}_monolithic_pair",
            f"v1lf_wing_{slug}_left_monolithic",
            f"v1lf_wing_{slug}_right_monolithic",
        )
        assembled_labels = (
            f"lx521_v1lf_basic_wing_{slug}_print_assembly",
            *(
                f"v1lf_wing_{slug}_{side}_{order}of3_{role}"
                for side in SIDE_NAMES
                for order, role in enumerate(PRINT_PART_ROLES, start=1)
            ),
        )
        ...
        review_relatives = [
            f"review/v1lf_wing_{slug}_{kind}.png" for kind in REVIEW_KINDS
        ]
        ...
        for side, role, order in sorted(expected_keys):
            ...
            assert assembly_bbox["min_mm"][2] >= REAR_Z_MM - 0.02, (
                f"{slug} {side}/{role}: extends behind V1LF envelope")
        ...
        assert canonical_record == {
            ...
            "label": f"lx521_v1lf_basic_wing_{slug}_monolithic_pair",
            "solid_count": 2,
            "bbox_mm": canonical_bbox,
        }
        assert assembled_record == {
            "path": assembled_relative,
            "label": f"lx521_v1lf_basic_wing_{slug}_print_assembly",
            "solid_count": 6,
...
def test_live_brep_geometry_contract() -> None:
    ...
    assert [record["label"] for record in context_records] == [
        "V1LF LM lower — floor stand",
        "V1LF LM lower — no-floor",
        "V1LF LM upper — no-floor reference",
        "V1LF UM — no-floor reference",
        "V1LF T crescent — no-floor reference",
    ]
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Remote-only Ac/Ae V1LF wing acceptance")
    parser.add_argument("--artifact-root", default="wings")
    args = parser.parse_args()
    artifact_root = Path(args.artifact_root)
    if not artifact_root.is_absolute():
        artifact_root = ROOT / artifact_root
    os.environ["LX_V1LF_BASIC_WING_ARTIFACT_ROOT"] = str(
        artifact_root.resolve())

    import run_memory_guarded as memory_guard

    if not memory_guard.is_guarded_process():
        guard = ROOT / "run_memory_guarded.py"
    ...
    single = os.environ.get("LX_V1LF_BASIC_WING_SINGLE_CHECK")
    if single:
    ...
    def run_check(check):
        env = os.environ.copy()
        env["LX_V1LF_BASIC_WING_SINGLE_CHECK"] = check.__name__
        completed = subprocess.run(
    ...
    print("\nall Ac/Ae V1LF wing checks passed")
    return 0
