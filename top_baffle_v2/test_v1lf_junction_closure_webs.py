"""Contracts for the V1LF full-depth LM--UM and T--UM closure webs.

The cheap source test runs without importing OCC.  Analytic/BREP checks are
guarded because released V1LF carrier construction is remote-only.  Run the
...
CORE_SOURCE = ROOT / "top_baffle_nd25fw4_v1lf.py"
ADDON_SOURCE = ROOT / "top_baffle_nd25fw4_v1lf_attachments.py"
...
FULL_TEST_ENV = "LX_V1LF_CLOSURE_FULL_TEST"
PLAN_ONLY_ENV = "LX_V1LF_CLOSURE_PLAN_ONLY"
BASE_ONLY_ENV = "LX_V1LF_CLOSURE_BASE_ONLY"
DENSE_CASE_ENV = "LX_V1LF_CLOSURE_DENSE_CASE"
DENSE_SHARD_ENV = "LX_V1LF_CLOSURE_DENSE_SHARD"
DENSE_SHARD_COUNT = 4
DENSE_CASES = {
...
def _require_guarded_test() -> None:
    assert _guarded_build(), (
        "V1LF analytic/BREP acceptance requires guarded remote CAD")
...
def test_make_jobserver_owns_the_complete_dense_matrix() -> None:
    ...
    assert "LX_V1LF_CLOSURE_BASE_ONLY=1" in makefile
    assert "LX_V1LF_CLOSURE_DENSE_CASE=$(1)" in makefile
    assert "LX_V1LF_CLOSURE_DENSE_SHARD=$(2)/4" in makefile
    tree = ast.parse(source)
    imported = {
        alias.name
...
def test_analytic_closure_plans_have_only_open_assembly_seams() -> None:
    ...
    import top_baffle_nd25fw4_v1lf as cad
...
def test_brep_web_prisms_are_solid_through_the_complete_v1lf_depth() -> None:
    _require_guarded_test()
    import top_baffle_nd25fw4_v1lf as cad
...
@lru_cache(maxsize=2)
def v1lf_actual_parts(stand_foot: bool = False):
    ...
    manifest = ROOT / state / ".v1lf_stage/manifest.json"
...
def _cropped_junction_parts(stand_foot: bool, junction: str):
    ...
    actual = v1lf_actual_parts(stand_foot)
    return {
...
FULL_CHECKS = (
    ...
    test_brep_web_prisms_are_solid_through_the_complete_v1lf_depth,
...
def main() -> int:
    ...
    if not _guarded_build():
        ...
        print(
            ...
            "remote CAD guard or set LX_V1LF_CLOSURE_FULL_TEST=1 to require "
            "them",
            flush=True,
        )
        return 0
