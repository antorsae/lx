...
This module intentionally does not import ``top_baffle_nd25fw4_v1lf`` so the
...
def _require_guarded_build() -> None:
    ...
    if not memory_guard.is_guarded_process():
        raise RuntimeError(
            "integral V1LF floor geometry requires run_memory_guarded.py")
...
def integrated_floor_addition():
    ...
    if not STAND_FOOT:
        raise RuntimeError("no-floor V1LF has no integral floor addition")
    foot = Pos(
        0.0,
...
def integrated_floor_feature_group(index: int):
    ...
    _require_guarded_build()
    if not STAND_FOOT:
        raise RuntimeError("no-floor V1LF has no integral floor features")
    if index == 0:
        cavity = Pos(
            0.0,
