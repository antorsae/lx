#!/usr/bin/env python3
"""Dimensioned printable-layout sheet for V1LF Ac/Ae/B1/B2 attachments.

This is deliberately a 2-D, source-linked engineering drawing generator.  It
uses the exact current A-comp, B1 and B2 outer profiles and the exact V1LF
carrier/magnet datums.  Ac/Ae release geometry is generated separately by
``v1lf_basic_wings_cad.py`` from this same analytic contract; this sheet
documents its print envelope, segmentation, receiver-pad correction, Ac
constant-depth reference and boundary-aware Ae LM/UM/T-weighted rear field.

Ordinary use is remote-first through the repository Makefile::

    make v1lf_basic_variants
"""
...
# protected V1LF contact lands retain the full 11.5-mm envelope.  The three
...
# Some full-depth V1LF mating bands approach the A outline more closely than
...
VARIANTS = (
    VariantDefinition("A", "A-V1LF requested straight upper taper", OUTLINE_A_COMP),
    VariantDefinition("B1", "B1", OUTLINE_B1),
    VariantDefinition("B2", "B2-V1LF local receiver correction", OUTLINE_B2),
)
...
def _effective_profile(definition: VariantDefinition) -> Polygon:
    """Return the attachment profile after explicit V1LF-only corrections.
    ...
    it runs from B2's source top-stub endpoint (49.177, 453.457) to the
    unchanged A crest (60.654, 391.709), mirrored on the left.  B1 stays exact;
    B2's positive UM receiver correction is added later by the root builder.
    """
    ...
    if effective.geom_type != "Polygon" or not effective.is_valid:
        raise RuntimeError("A-V1LF tapered profile must resolve to one polygon")
    return effective
...
def _intercarrier_bridge_right() -> Polygon:
    """Outboard positive-area spine across the B2 carrier pinch.

    The historic B2 edge crosses inside the R51.7 V1LF carrier through part
    ...
    the minimum print-realizable connection; it stays outside both rings and
    is included in B2's explicitly drawn local correction.
    """
    points = (
...
def _common_keepout_parts() -> dict[str, Polygon]:
    """Named V1LF plan envelopes that an attachment can contact.
...
def _receiver_root(profile: Polygon, site, key: str) -> tuple[Polygon, Polygon]:
    ...
    if is_um:
        ...
        if key != "B2":
            ...
            correction = root.difference(profile).buffer(0)
    else:
        root = pad.intersection(profile.buffer(0.01)).buffer(0)
    return root, correction


def _field_for_variant(definition: VariantDefinition) -> tuple[Polygon, Polygon, Polygon]:
    ...
    for name in ACTIVE_RECEIVER_NAMES_RIGHT:
        root, correction = _receiver_root(profile, sites[name], definition.key)
        roots.append(root)
        if not correction.is_empty:
            corrections.append(correction)
    spine = _intercarrier_bridge_right()
    if definition.key != "B2":
        spine = spine.intersection(profile).buffer(0)
    field = unary_union((field, *roots, spine)).buffer(0)
    if definition.key == "B2":
        corrections.append(spine.difference(profile).buffer(0))
    field = _largest_polygon(field)
    correction = (unary_union(corrections).buffer(0)
                  if corrections else Polygon())
    return profile, field, correction
...
def _build_layout(definition: VariantDefinition) -> VariantLayout:
    profile, field, correction = _field_for_variant(definition)
    ...
    layout = VariantLayout(definition, profile, field, nominal_parts,
                           print_parts, structure, correction,
...
def _validate_layout(layout: VariantLayout) -> None:
    ...
    if layout.definition.key == "B2" and layout.b2_correction.area < 10.0:
        raise RuntimeError("B2 must contain a positive local UM correction")
...
def _ae_protected_region(
    ...
    sites = _right_sites()
    # Receiver protection follows the actual surface-normal envelopes. Every V1LF
    # mating boundary is protected separately below; larger construction fans
    # that do not touch V1LF may still follow the weighted rear field.
    roots = tuple(
    ...
    for name, keepout in named_keepouts.items():
        # White in the plan is open V1LF envelope, but the adjoining Ae
    ...
    if contact_land.is_empty:
        raise RuntimeError("Ae has no full-depth V1LF contact land")
...
def _build_ae_depth_field(
    ...
    if (np.any(contact_mask)
        ...
        raise RuntimeError("Ae V1LF contact land is not full depth")
...
def _draw_variant(
    ...
    if not layout.b2_correction.is_empty:
        corr = unary_union((layout.b2_correction, _mirror(layout.b2_correction)))
        _draw_shape(ax, corr, facecolor="none", edgecolor=COLORS["correction"],
    ...
    if labels:
        ...
        ax.text(-157, 575,
                ...
                "red: printable A-V1LF taper; dotted red: nominal extension",
...
def _draw_ae_depth_map(
    ...
    ax.annotate(
        "WHITE = open V1LF carrier / joint / T-crescent keepout\n"
...
def _draw_b2_detail(ax, layout: VariantLayout):
    site = _right_sites()["um_right"]
    center = np.asarray(site["center"], dtype=float)
    normal = np.asarray(site["normal"], dtype=float)
    tangent = np.array((-normal[1], normal[0]))
    face = np.asarray(site["face"], dtype=float)
    profile_line = layout.profile.boundary
    ray = LineString((tuple(center), tuple(center + 100.0 * normal)))
    hit = profile_line.intersection(ray)
    hit_points = []
    if hit.geom_type == "Point":
        hit_points = [hit]
    elif hasattr(hit, "geoms"):
        hit_points = [g for g in hit.geoms if g.geom_type == "Point"]
    radii = [Point(*center).distance(point) for point in hit_points
             if Point(*center).distance(point) >= UM_CORE_R - 0.1]
    historic_r = min(radii) if radii else 51.942

    carrier = Point(*UM_CENTER).buffer(UM_CORE_R, resolution=128)
    _draw_shape(ax, carrier, facecolor=COLORS["carrier"],
                edgecolor=COLORS["carrier_edge"], linewidth=1.0, zorder=1)
    _draw_shape(ax, layout.field_right.intersection(box(18, 390, 55, 427)),
                facecolor=COLORS["um"], edgecolor="#9b5a28",
                linewidth=0.8, alpha=0.8, zorder=2)
    p = np.asarray(layout.profile.exterior.coords)
    ax.plot(p[:, 0], p[:, 1], color=COLORS["historic"], lw=1.2,
            ls=(0, (5, 3)), zorder=4, label="historic B2")
    pocket = _pocket_plan(site)
    _draw_shape(ax, pocket, facecolor=COLORS["active_mag"],
                edgecolor="white", linewidth=0.5, alpha=0.85, zorder=5)
    ax.plot((face[0], face[0] + PAD_UM_RADIAL_MM * normal[0]),
            (face[1], face[1] + PAD_UM_RADIAL_MM * normal[1]),
            color=COLORS["correction"], lw=1.0, marker="|", zorder=7)
    outer = face + PAD_UM_RADIAL_MM * normal
    ax.annotate("8.0 mm nominal radial pad",
                xy=outer, xytext=(21.0, 424.5), fontsize=7.0,
                color=COLORS["correction"],
                arrowprops=dict(arrowstyle="->", color=COLORS["correction"],
                                lw=0.8))
    hist = center + historic_r * normal
    ax.annotate(f"historic radial land = {historic_r - UM_CORE_R:.3f} mm\n"
                "receiver axis exits B2",
                xy=hist, xytext=(19.0, 391.5), fontsize=6.8,
    ...
    ax.annotate("D5.2 x 2.2 receiver\nfor D5 x 2 magnet",
                ...
                arrowprops=dict(arrowstyle="->", color=COLORS["active_mag"],
                                lw=0.8))
    ax.text(19.0, 427.0,
            "B2 correction is local and carrier-following;\n"
            "B1 stays exact; A uses the requested straight taper.",
            fontsize=6.8, va="bottom", color="#333b41")
    ax.set_title("B2-V1LF receiver correction — exact UM root", fontsize=9.5,
                 weight="bold")
    ax.set_aspect("equal")
    ax.set_xlim(17, 56)
    ax.set_ylim(389, 432)
...
def _draw_depth_and_table(
        ax, layouts: tuple[VariantLayout, ...],
    ...
    seam_contract = (
        ...
        f"{layouts[0].metrics['lower_key_ligament_mm']:.2f}; U 7/8.5/4, "
        f"ligament {layouts[0].metrics['upper_key_ligament_mm']:.2f} mm")
    ax.text(ox + ow / 2.0, oy - 2.0, seam_contract,
            fontsize=5.65, ha="center", va="top")

    ax.text(4.0, 57.5,
            "Ac/Ae are solid. Ae stays full-depth at every V1LF carrier, "
    ...
    a_layout, b1_layout, b2_layout = layouts
    rows = (
        ("Ac", "solid flat rear", "11.5",
         f"{depth_field.ac_mass_g:.0f}", a_layout),
        ("Ae", "weighted solid rear", f"{solution.edge_depth_mm:.2f}..11.5",
         f"{depth_field.mass_g:.0f}", a_layout),
        ("B1", "open skin+ribs", "1.2 / 11.5",
         f"{b1_layout.metrics['mass_g_side']:.0f}", b1_layout),
        ("B2", "open skin+ribs", "1.2 / 11.5",
         f"{b2_layout.metrics['mass_g_side']:.0f}", b2_layout),
    )
...
def render(output: Path):
    ...
    top_axes = [fig.add_subplot(gs[0, index]) for index in range(4)]
    _draw_variant(
        top_axes[0], a_layout, labels=True,
    ...
    _draw_variant(
        top_axes[2], b1_layout, display_title="B1 — exact source plan",
        construction_text=(
            f"rear-open 1.2 skin + 2.4 ribs | "
            f"~{b1_layout.metrics['mass_g_side']:.0f} g/side"))
    _draw_variant(
        top_axes[3], b2_layout,
        display_title="B2 — local V1LF receiver correction",
        construction_text=(
            f"rear-open 1.2 skin + 2.4 ribs | "
            f"~{b2_layout.metrics['mass_g_side']:.0f} g/side"))
    ...
    b2_ax = fig.add_subplot(gs[2, 2])
    _draw_b2_detail(b2_ax, layouts[2])
    contract_ax = fig.add_subplot(gs[2, 3])
    _draw_depth_and_table(contract_ax, layouts, solution, depth_field)

    fig.suptitle(
        "V1LF PRINTABLE Ac / Ae / B1 / B2 ATTACHMENTS — ENGINEERING LAYOUT",
    ...
    fig.text(
        ...
        "flush-filled "
        "V1L-style dovetail seams; "
        "B1/B2 retain rear-open construction",
        ha="center", va="center", fontsize=9.0, color="#3f474d")
    fig.text(
        0.5, 0.018,
        "CAD-LINKED PRINTABLE LAYOUT — Ac/Ae STEP + six STLs each in wings/; "
    ...
    try:
        fig.savefig(
            temporary,
            metadata={
                "Title": "V1LF printable Ac/Ae/B1/B2 engineering layout",
                "Description": (
                    ...
                    "source B1/B2 plans; B2 local receiver "
                    "correction; CAD-linked engineering layout"),
            },
        )
        plt.close(fig)
