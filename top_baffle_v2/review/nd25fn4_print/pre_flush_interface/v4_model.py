...
def tweeter_outer(x,y,z):
    ...
    return np.maximum(radial,abs(x)-p.overall_depth/2)
...
def um_rim_height(vertical):
    return UM_RIM_RISE*(1-smooth01((vertical-396.)/22.))
...
def um_front_rise(lateral,vertical):
    """Broad bowl face and an integrated base over the retained LM mounts."""
...
def rear_height(lateral,vertical,field=tweeter_outer):
    ...
    high=np.full(lateral.shape,4.5)
    ...
    return np.where(valid,(low+high)/2,4.5)
...
def envelope_field(x,y,z):
    ...
    return result+weight*(np.maximum(result,rear_limit)-result)
...
def housing_field(x,y,z):
    ...
    for i in (0,1):
        ...
        m=retained.M
        for radius,hi in [(m.service_radius,m.service_end_x),(m.flange_cavity_radius,m.seat_x),
...
def fused_body(upper):
    ...
    # The reference base overlaps the LM only in projection. Its new
    # front apron clears the existing front face by 0.25 mm.
    lm_clearance^=md.Manifold.cube((240,240,UM_APRON_REAR_Z+20)).translate((-120,260,-20))
    blank-=lm_clearance
    # The bowed front corners also pass over the ends of the existing
    # upper wings. Keep their underside ahead of Z18.3 by 0.25 mm, then
    # ease back into the original rear sweep above that overlap. Native
    # receiver lands are restored separately below.
    def apron_rear_clearance(x,y,z):
        rear=um_rear(y,x)
        height=rear+(UM_APRON_REAR_Z-rear)*(1-smooth01((y-310.1)/4.))
        return np.maximum.reduce(np.broadcast_arrays(
            z-height,abs(x)-49.,297.-y,y-315.5,-20.-z))
    blank-=solid(extract_surface(apron_rear_clearance,
        [[-50,50],[296,316],[-21,20]],.2,[]))
    # Continue the existing solid front lands into the new apron. This
    # closes a potential 0.25-mm air shelf above an UM receiver without
    # adding material behind, or inside, any LM mating face.
    front_lands=exact_original.slice(18.29).extrude(5.).translate((0,0,18.29))
    blank+=full_blank^front_lands
    ...
    lm_front_shadow=lm_front.project().offset(.05).extrude(UM_APRON_REAR_Z-18.27).translate((0,0,18.27))
