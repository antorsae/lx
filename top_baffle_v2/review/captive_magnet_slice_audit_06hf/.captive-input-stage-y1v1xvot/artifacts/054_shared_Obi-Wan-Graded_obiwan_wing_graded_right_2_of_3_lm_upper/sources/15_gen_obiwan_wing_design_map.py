...
# The straight seam slit tapers closed over the last 2 mm at each exposed
# endpoint so graded retains one continuous constant-thickness acoustic knife
# edge.
#
# The male key is a constant prism; the female relief is depth-varying.  Its
# clearance grows linearly from the front face to the rear, so the slide
# assembly enters from the rear through a progressively tighter fit and ends
# tight where the seam is visible, and the widening rear half is itself the
# two-part epoxy gallery.  Zero front clearance is the dry land: the gap
# closes to the calibrated fit before it can reach the visible front seam.
# Retention is the cured adhesive in shear, not friction, because PLA creeps
# out of any stored elastic strain within days.
#
# The front value is NOT zero, and the reason is worth stating: a wedge
# amplifies print oversize by the inverse of its taper.  A male key printed
# oversize by e cannot pass the station where the clearance equals e, so it
# stops 11.5 * (e - front) / (rear - front) mm proud -- 57 mm per mm of
# oversize at a 0.00 front.  Coplanar wing panels must finish flush, so that
# is a hard failure.  0.05 is the coupon-calibrated clearance the previous
# uniform joint assembled with, so the front of the wedge is no tighter than
# a fit these parts are known to make, while the rear still opens the full
# 0.20 gallery.  Lower it only against a measured oversize from
# lx521_coupon_1_fit_plate / _2_fit_key, never by preference.
DOVETAIL_FRONT_CLEARANCE_MM = 0.05
DOVETAIL_REAR_CLEARANCE_MM = 0.20
# Anchor the linear law this far in FRONT of the front face.  The apex then
# sits outside the solid, so the relief loft crosses the front datum
# transversally instead of touching it tangentially, at the cost of
# 0.20 * 0.02 / 11.5 = 0.35 um of front-face clearance.
DOVETAIL_TAPER_APEX_OFFSET_MM = 0.02
DOVETAIL_ENDPOINT_TAPER_MM = 2.0
# Let the endpoint law cross zero and retreat this far into the male instead
# of grazing the seam plane.  The relief then starts and ends on a clean
# transversal crossing rather than an asymptotically thin sliver, and each
# exposed edge keeps a short fully-closed dry land.
DOVETAIL_ENDPOINT_RETREAT_MM = 0.05
...
# Serpentine adhesive channel on the female flank.  It is a groove in the
# female wall only; the male key stays a constant prism, so the channel adds
# no envelope and no front-face feature.  Cured epoxy in the groove bears
# directly against the groove's two slide-facing walls, which is what stops
# the Z slide once friction has crept away.
#
# Everything is referenced to the LOCAL rear surface, because the graded rear
# plunges from 11.5 mm to one 0.24-mm layer along these very seams.  A band
# fixed to the flat rear datum would leave the material entirely.
DOVETAIL_CHANNEL_REAR_LAND_MM = 1.20
DOVETAIL_CHANNEL_HEIGHT_MM = 2.20
DOVETAIL_CHANNEL_DEPTH_MM = 0.80
# The groove's rear-facing wall is the one printed over air (the pieces print
# front-down).  Close it on a 45-degree ramp measured in the female material,
# which for a t_out overshoot of DOVETAIL_CHANNEL_GAP_OVERSHOOT_MM means a
# ramp run of depth + overshoot.
DOVETAIL_CHANNEL_GAP_OVERSHOOT_MM = 0.25
DOVETAIL_CHANNEL_AMPLITUDE_MM = 0.50
DOVETAIL_CHANNEL_WAVELENGTH_MM = 7.0
DOVETAIL_CHANNEL_FRONT_LAND_MM = 1.60
# Overflow traps: the channel's first and last run deepen and grow taller so
# excess resin has somewhere to go instead of being driven at the dry land.
DOVETAIL_TRAP_LENGTH_MM = 2.00
DOVETAIL_TRAP_EXTRA_DEPTH_MM = 0.40
DOVETAIL_TRAP_EXTRA_HEIGHT_MM = 0.80
# Local panel depth needed to host land + height + full serpentine swing +
# trap growth + front land.  Below it the channel simply stops and the
# tapered gallery carries the joint on its own.
DOVETAIL_CHANNEL_MIN_LOCAL_DEPTH_MM = (
    DOVETAIL_CHANNEL_REAR_LAND_MM + DOVETAIL_CHANNEL_HEIGHT_MM
    + 2.0 * DOVETAIL_CHANNEL_AMPLITUDE_MM + DOVETAIL_TRAP_EXTRA_HEIGHT_MM
    + DOVETAIL_CHANNEL_FRONT_LAND_MM)
# Keep the channel this far along the contour from each exposed edge so both
# seam endpoints keep a solid dry land, and never cut the female wall thinner
# than the established 2.0-mm outer ligament gate.
DOVETAIL_CHANNEL_END_INSET_MM = 3.0
DOVETAIL_CHANNEL_MIN_RUN_MM = 6.0
DOVETAIL_CHANNEL_MIN_DEPTH_MM = 0.35
DOVETAIL_CHANNEL_SAMPLE_MM = 0.35
DOVETAIL_RELIEF_SAMPLE_MM = 0.10
# The relief sweep is backed this far into already-solid male material so the
# tool is a closed solid rather than a sheet.  Keep it small: the mitred
# section at the key's 104-degree corners reaches 1.3 back-widths along the
# contour, and nothing else may be sampled inside that reach.
DOVETAIL_RELIEF_BACK_MM = 0.25
# Start and end the sweep where its outer wall is already this far inside the
# male, so it opens on a transversal crossing instead of grazing the seam.
DOVETAIL_RELIEF_ENTRY_BITE_MM = 0.02
# The channel carries no dedicated rear-face port or vent, and the reason is
# geometric rather than a preference.  Any opening from the rear face into
# the buried channel has to sit immediately alongside the rear seam gap,
# which is itself open the whole length of the joint; the land left between
# those two openings is narrower than a printable wall wherever their
# boundaries meet, and reads -- correctly -- as a standing membrane.  Both a
# bored port and a slot cut by the channel itself were built and measured,
# and both left exactly that fin.  The joint is instead injected through the
# rear seam gap, which the depth taper opens to
# DOVETAIL_REAR_CLEARANCE_MM along every millimetre of the seam and which
# the channel meets through its own flank, so the same gap feeds it and
# vents it.
...
class VariantLayout:
    ...
    two_piece_nominal_parts: dict[str, Polygon]
...
def _straight_joint_seam(
    ...
    trapezoidal male key supplies the XY interlock; its female receiver on the
    next-higher print carries the depth-varying relief that turns the fit into
    a bonded joint instead of a friction one.
    """
    x0, x1 = -100.0, 250.0
    baseline = LineString((
...
def dovetail_plan_clearance_mm(endpoint_distance_mm):
    """Rear-face female clearance as a function of distance from an edge.

    Positive is outward into the female.  The law deliberately crosses zero
    and retreats into the male at each exposed end so the relief opens on a
    transversal crossing and both acoustic edges keep a closed dry land.
    """
    blend = _smootherstep01(
        np.asarray(endpoint_distance_mm, dtype=float)
        / DOVETAIL_ENDPOINT_TAPER_MM)
    return (DOVETAIL_REAR_CLEARANCE_MM * blend
            - DOVETAIL_ENDPOINT_RETREAT_MM * (1.0 - blend))


def dovetail_endpoint_dry_land_mm() -> float:
    """Contour run at each exposed end where the rear clearance is zero."""
    blend = DOVETAIL_ENDPOINT_RETREAT_MM / (
        DOVETAIL_REAR_CLEARANCE_MM + DOVETAIL_ENDPOINT_RETREAT_MM)
    grid = np.linspace(0.0, DOVETAIL_ENDPOINT_TAPER_MM, 20001)
    return float(np.interp(
        blend, _smootherstep01(grid / DOVETAIL_ENDPOINT_TAPER_MM), grid))


def dovetail_depth_clearance_scale(z_mm, front_z_mm: float):
    """Linear front-to-rear scale applied to every plan clearance.

    One knob per end: the scale reaches
    ``DOVETAIL_FRONT_CLEARANCE_MM / DOVETAIL_REAR_CLEARANCE_MM`` at the front
    datum and 1 at the rear.  With a zero front clearance that is simply the
    fraction of the depth travelled from the front.
    """
    fraction = ((float(front_z_mm) + DOVETAIL_TAPER_APEX_OFFSET_MM
                 - np.asarray(z_mm, dtype=float)) / DEPTH_MM)
    return (DOVETAIL_FRONT_CLEARANCE_MM
            + (DOVETAIL_REAR_CLEARANCE_MM - DOVETAIL_FRONT_CLEARANCE_MM)
            * fraction) / DOVETAIL_REAR_CLEARANCE_MM


def _tapered_female_slit(
        seam: LineString, normal: np.ndarray, field: Polygon,
        *, samples: int = 241) -> Polygon:
    """One-sided rear-face female slit, closed at both exposed endpoints."""
    endpoints = np.asarray((seam.coords[0], seam.coords[-1]), dtype=float)
    vector = endpoints[1] - endpoints[0]
    ...
    widths = np.maximum(
        dovetail_plan_clearance_mm(endpoint_distance), 0.0)
...
def _female_clearance_gap(
    ...
    """Return the rear-face female relief without opening the acoustic edge.

    This is the plan footprint of the widest section of the depth-varying
    relief.  The solid relief lofts from zero at the front face out to this
    footprint at the rear; only the drawing and the plan gates use it.
    """
...
DOVETAIL_OWNER_PAIRS = (("lm_lower", "lm_upper"), ("lm_upper", "um"))


def dovetail_joint_contour(layout, index: int) -> LineString:
    """Return the shared male/female boundary of one joint, end to end.

    The result is ordered from the seam's first exposed endpoint to its
    second, and runs along the seam, out around the male key and back.
    """
    male_owner, female_owner = DOVETAIL_OWNER_PAIRS[index]
    shared = layout.nominal_parts[male_owner].boundary.intersection(
        layout.nominal_parts[female_owner].boundary)
    merged = linemerge(shared)
    if merged.geom_type != "LineString":
        raise RuntimeError(
            f"dovetail {index} shared boundary is {merged.geom_type}, "
            "not one continuous contour")
    coords = np.asarray(merged.coords, dtype=float)
    start = np.asarray(layout.joint_seams[index].coords[0], dtype=float)
    if (np.linalg.norm(coords[0] - start)
            > np.linalg.norm(coords[-1] - start)):
        coords = coords[::-1]
    return LineString(coords)


def _contour_corners(contour: LineString) -> np.ndarray:
    """Arclength of every genuine direction change along one contour."""
    coords = np.asarray(contour.coords, dtype=float)
    steps = np.linalg.norm(np.diff(coords, axis=0), axis=1)
    stations = np.concatenate(([0.0], np.cumsum(steps)))
    directions = np.diff(coords, axis=0) / steps[:, None]
    turns = np.einsum("ij,ij->i", directions[:-1], directions[1:])
    return stations[1:-1][turns < 1.0 - 1.0e-9]


def _sweep_frames(layout, index: int, contour: LineString,
                  stations, corner_keepout_mm: float) -> dict:
    """Order one tool's sections and mitre them through the key's corners.

    Every section lies in the plane spanned by its own frame vector and Z.
    On a straight run the frame vector is the unit female-facing normal; at a
    corner it is the bisector divided by ``cos`` of the half angle, which is
    exactly the mitre a swept prism needs.  Stations inside
    ``corner_keepout_mm`` of a corner are dropped so the mitred section
    cannot fold back through its neighbours.
    """
    corners = _contour_corners(contour)
    keep = [float(value) for value in np.asarray(stations, dtype=float)
            if not len(corners)
            or float(np.min(np.abs(corners - value))) > corner_keepout_mm]
    ordered = np.unique(np.concatenate((np.asarray(keep, dtype=float),
                                        corners)))
    low, high = float(np.min(stations)), float(np.max(stations))
    ordered = ordered[(ordered >= low - 1.0e-9) & (ordered <= high + 1.0e-9)]
    coords = np.array(
        [[contour.interpolate(float(value)).x,
          contour.interpolate(float(value)).y] for value in ordered],
        dtype=float)
    sign = _female_side_sign(layout, index, contour)
    frames = np.zeros_like(coords)
    for position, value in enumerate(ordered):
        before = _contour_direction(contour, value, -1.0)
        after = _contour_direction(contour, value, +1.0)
        normal_before = sign * np.array((-before[1], before[0]))
        normal_after = sign * np.array((-after[1], after[0]))
        bisector = normal_before + normal_after
        norm = float(np.linalg.norm(bisector))
        if norm <= 1.0e-9:
            frames[position] = normal_after
            continue
        bisector /= norm
        frames[position] = bisector / float(np.dot(bisector, normal_after))
    return {"station_mm": ordered, "xy_mm": coords, "frame_xy": frames,
            "corner_mm": corners}


def _contour_direction(contour: LineString, station: float,
                       sign: float) -> np.ndarray:
    """Unit direction of the contour just before or just after a station."""
    step = 1.0e-4
    low = max(0.0, min(contour.length, station + min(0.0, sign) * step))
    high = max(0.0, min(contour.length, station + max(0.0, sign) * step))
    if high - low <= 1.0e-9:
        low, high = (max(0.0, station - step), min(contour.length,
                                                   station + step))
    start = contour.interpolate(low)
    end = contour.interpolate(high)
    direction = np.array((end.x - start.x, end.y - start.y), dtype=float)
    return direction / float(np.linalg.norm(direction))


def _female_side_sign(layout, index: int, contour: LineString) -> float:
    """Which way ``rotate(tangent, +90)`` faces along one whole contour.

    The contour is a single continuous male/female boundary, so the male
    stays on one side of it throughout and one sign settles every station.
    Probing at a corner would not do: perpendicular to the seam at the key's
    neck lands inside the key and reads the male on the wrong side.
    """
    coords = np.asarray(contour.coords, dtype=float)
    steps = np.linalg.norm(np.diff(coords, axis=0), axis=1)
    longest = int(np.argmax(steps))
    middle = 0.5 * (coords[longest] + coords[longest + 1])
    tangent = (coords[longest + 1] - coords[longest]) / steps[longest]
    normal = np.array((-tangent[1], tangent[0]), dtype=float)
    male = layout.nominal_parts[DOVETAIL_OWNER_PAIRS[index][0]]
    probe = Point(*(middle + 0.05 * normal))
    return -1.0 if male.contains(probe) else 1.0


def dovetail_relief_sweep(layout, index: int) -> dict:
    """Return the depth-varying female relief of one joint as one sweep.

    The sweep is trimmed to the span where the plan clearance is actually
    positive, so it opens and closes on a clean transversal crossing of the
    seam plane and leaves both exposed acoustic edges fully closed.  ``wings``
    sweeps a trapezoid whose outer edge runs from ``clearance * scale(rear)``
    at the rear to ``clearance * scale(front)`` at the front; that single
    solid is the entire relief, and the male key stays a constant prism.
    """
    contour = dovetail_joint_contour(layout, index)
    length = contour.length
    entry = _relief_entry_mm()
    stations = {entry, length - entry}
    for station in np.arange(0.0, length, DOVETAIL_RELIEF_SAMPLE_MM):
        if entry < station < length - entry and (
                min(station, length - station)
                <= DOVETAIL_ENDPOINT_TAPER_MM):
            stations.add(float(station))
    for corner in _contour_corners(contour):
        if entry < corner < length - entry:
            stations.add(float(corner))
    reach = (DOVETAIL_RELIEF_BACK_MM + DOVETAIL_REAR_CLEARANCE_MM) * 1.3
    frames = _sweep_frames(
        layout, index, contour, np.array(sorted(stations), dtype=float),
        reach)
    clearance = dovetail_plan_clearance_mm(
        np.minimum(frames["station_mm"], length - frames["station_mm"]))
    peak = float(np.max(clearance))
    if not math.isclose(peak, DOVETAIL_REAR_CLEARANCE_MM, abs_tol=1.0e-6):
        raise RuntimeError(
            f"dovetail {index} relief peaks at {peak:.4f} mm instead of the "
            f"{DOVETAIL_REAR_CLEARANCE_MM:.4f}-mm rear clearance")
    return {
        "name": layout.dovetail_keys[index]["name"],
        "male_owner": DOVETAIL_OWNER_PAIRS[index][0],
        "female_owner": DOVETAIL_OWNER_PAIRS[index][1],
        "contour": contour,
        "contour_mm": float(length),
        "station_mm": frames["station_mm"],
        "xy_mm": frames["xy_mm"],
        "frame_xy": frames["frame_xy"],
        "clearance_mm": clearance,
        "back_mm": DOVETAIL_RELIEF_BACK_MM,
        "entry_mm": entry,
        "endpoint_dry_land_mm": dovetail_endpoint_dry_land_mm(),
    }


def _relief_entry_mm() -> float:
    """Station where the relief starts, just inside the male."""
    grid = np.linspace(0.0, DOVETAIL_ENDPOINT_TAPER_MM, 20001)
    law = dovetail_plan_clearance_mm(grid)
    return float(np.interp(-DOVETAIL_RELIEF_ENTRY_BITE_MM, law, grid))


def dovetail_sweep_plan(sweep: dict, scale: float, *,
                        outer_key: str = "clearance_mm",
                        back_key: str = "back_mm") -> Polygon:
    """Plan footprint of one swept tool at a given depth scale."""
    outer_offset = np.asarray(sweep[outer_key], dtype=float) * scale
    back = sweep[back_key]
    if np.ndim(back) == 0:
        back = np.full(len(outer_offset), float(back))
    frames = sweep["frame_xy"]
    base = sweep["xy_mm"]
    outer = base + outer_offset[:, None] * frames
    inner = base - np.asarray(back, dtype=float)[:, None] * frames
    return Polygon(np.vstack((inner, outer[::-1]))).buffer(0)
...
def _channel_window(
        contour: LineString, field: Polygon, local_depth) -> tuple[float,
                                                                   float]:
    """Longest contour run that can host the channel at full geometry."""
    length = contour.length
    inset = DOVETAIL_CHANNEL_END_INSET_MM
    if length <= 2.0 * inset + DOVETAIL_CHANNEL_MIN_RUN_MM:
        return (0.0, 0.0)
    grid = np.arange(inset, length - inset + 1.0e-9,
                     0.5 * DOVETAIL_CHANNEL_SAMPLE_MM)
    points = [contour.interpolate(float(value)) for value in grid]
    coords = np.array([[point.x, point.y] for point in points], dtype=float)
    depth = np.asarray(local_depth(coords), dtype=float)
    ligament = np.array(
        [point.distance(field.boundary) for point in points], dtype=float)
    # The groove is measured from the female flank, which has already stepped
    # off the nominal plane by up to the full rear clearance, so charge that
    # to the ligament budget here rather than discovering it downstream.
    usable = np.minimum(
        DOVETAIL_CHANNEL_DEPTH_MM,
        ligament - DOVETAIL_MIN_LIGAMENT_MM - DOVETAIL_REAR_CLEARANCE_MM)
    good = ((depth >= DOVETAIL_CHANNEL_MIN_LOCAL_DEPTH_MM)
            & (usable >= DOVETAIL_CHANNEL_MIN_DEPTH_MM))
    best = (0.0, 0.0)
    index = 0
    while index < len(good):
        ...
        run = float(grid[end] - grid[index])
        if run > best[1] - best[0]:
            best = (float(grid[index]), float(grid[end]))
        index = end + 1
    if best[1] - best[0] < DOVETAIL_CHANNEL_MIN_RUN_MM:
        return (0.0, 0.0)
    return best


def _channel_sections(contour: LineString, field: Polygon, local_depth,
                      start: float, end: float, wavelength: float,
                      scale: float, frames: dict) -> dict:
    """Section parameters of the channel at one sweep's own stations."""
    stations = frames["station_mm"]
    coords = frames["xy_mm"]
    depth = np.asarray(local_depth(coords), dtype=float)
    ligament = np.array(
        [Point(*point).distance(field.boundary) for point in coords],
        dtype=float)
    blend = 1.0 - _smootherstep01(
        np.minimum(stations - start, end - stations) / DOVETAIL_TRAP_LENGTH_MM)
    height = (DOVETAIL_CHANNEL_HEIGHT_MM
              + DOVETAIL_TRAP_EXTRA_HEIGHT_MM * blend)
    wave = np.sin(2.0 * math.pi * (stations - start) / wavelength)
    # Referencing the centre to the local height keeps the rear land exact by
    # construction, including where a trap has grown the section.
    centre = (DOVETAIL_CHANNEL_REAR_LAND_MM + 0.5 * height
              + DOVETAIL_CHANNEL_AMPLITUDE_MM * (1.0 + wave))
    # The flank the groove is cut into has already stepped away from the
    # nominal seam by the depth-varying clearance, so measure the groove from
    # there and keep the ligament rule on the nominal plane where it belongs.
    flank = DOVETAIL_REAR_CLEARANCE_MM * dovetail_depth_clearance_scale(
        THICKNESS_MM - depth + centre, THICKNESS_MM)
    groove = scale * (DOVETAIL_CHANNEL_DEPTH_MM
                      + DOVETAIL_TRAP_EXTRA_DEPTH_MM * blend)
    cut_depth = np.minimum(
        flank + groove, ligament - DOVETAIL_MIN_LIGAMENT_MM)
    return {
        "local_depth_mm": depth,
        "ligament_mm": ligament,
        "flank_offset_mm": flank,
        "groove_depth_mm": cut_depth - flank,
        "cut_depth_mm": cut_depth,
        "height_mm": height,
        "ramp_mm": cut_depth + DOVETAIL_CHANNEL_GAP_OVERSHOOT_MM,
        "gap_overshoot_mm": DOVETAIL_CHANNEL_GAP_OVERSHOOT_MM,
        "centre_from_rear_mm": centre,
        "rear_edge_mm": centre - 0.5 * height,
        "front_edge_mm": centre + 0.5 * height,
    }


def dovetail_channel_sweep(
        layout, index: int, field: Polygon, local_depth) -> dict | None:
    """Return the serpentine adhesive channel of one joint, or ``None``.

    ``local_depth`` maps plan points to this variant's panel depth.  Every
    channel dimension is referenced to that local rear surface, because the
    graded rear plunges from the full 11.5 mm to a single 0.24-mm layer along
    these very seams; a band pinned to the flat rear datum would leave the
    material outright.  Where the panel is too shallow the channel simply
    stops and the tapered gallery carries the joint on its own.
    """
    contour = dovetail_joint_contour(layout, index)
    start, end = _channel_window(contour, field, local_depth)
    if end <= start:
        return None
    count = max(9, int(math.ceil(
        (end - start) / DOVETAIL_CHANNEL_SAMPLE_MM)) + 1)
    reach = (DOVETAIL_CHANNEL_DEPTH_MM + DOVETAIL_TRAP_EXTRA_DEPTH_MM) * 1.3
    frames = _sweep_frames(
        layout, index, contour, np.linspace(start, end, count), reach)
    # A window end inside a corner's mitre reach is snapped onto the corner,
    # so read the ends the sweep actually kept before phasing the wave and the
    # traps against them.  A whole number of periods puts both ends, and both
    # traps, on the serpentine's mid-line.
    start = float(frames["station_mm"][0])
    end = float(frames["station_mm"][-1])
    span = end - start
    periods = max(1, int(round(span / DOVETAIL_CHANNEL_WAVELENGTH_MM)))
    wavelength = span / periods

    # Surface the groove on the serpentine's rear-most excursions, where the
    # land above it is only DOVETAIL_CHANNEL_REAR_LAND_MM thick.  The first
    # is the injection port, the last is the vent; a channel with a single
    # excursion gets the port alone and vents through the seam gap, which is
    # open along the whole rear seam line anyway.
    # Square-free mitred joins still reach further into the female at a
    # corner than the nominal cut depth does, so gate the real footprint
    # against the ligament rule and back the channel off until it clears
    # instead of assuming the straight-run depth is safe.
    scale = 1.0
    female = layout.nominal_parts[DOVETAIL_OWNER_PAIRS[index][1]]
    for _attempt in range(24):
        section = _channel_sections(
            contour, field, local_depth, start, end, wavelength, scale,
            frames)
        footprint = dovetail_sweep_plan(
            {**frames, **section}, 1.0,
            outer_key="cut_depth_mm",
            back_key="gap_overshoot_mm").intersection(female)
        wall = float(footprint.distance(field.boundary))
        if wall >= DOVETAIL_MIN_LIGAMENT_MM - 1.0e-6:
            break
        scale *= 0.88
    else:
        raise RuntimeError(
            f"dovetail {index} channel cannot clear the "
            f"{DOVETAIL_MIN_LIGAMENT_MM:.2f}-mm ligament gate")
    groove = float(np.min(section["groove_depth_mm"]))
    if groove < DOVETAIL_CHANNEL_MIN_DEPTH_MM:
        raise RuntimeError(
            f"dovetail {index} channel groove is clamped to {groove:.3f} mm")
    rear_land = float(np.min(section["rear_edge_mm"]))
    if rear_land < DOVETAIL_CHANNEL_REAR_LAND_MM - 1.0e-6:
        raise RuntimeError(
            f"dovetail {index} channel leaves only {rear_land:.3f} mm of "
            "rear land")
    front_land = section["local_depth_mm"] - section["front_edge_mm"]
    if float(np.min(front_land)) < DOVETAIL_CHANNEL_FRONT_LAND_MM - 1.0e-6:
        raise RuntimeError(
            f"dovetail {index} channel leaves only "
            f"{float(np.min(front_land)):.3f} mm of front land")

    area = _channel_section_area(
        section["groove_depth_mm"], section["height_mm"])
    steps = np.linalg.norm(np.diff(frames["xy_mm"], axis=0), axis=1)
    rise = np.abs(np.diff(section["centre_from_rear_mm"]))
    lengths = np.hypot(steps, rise)
    return {
        "name": layout.dovetail_keys[index]["name"],
        "female_owner": DOVETAIL_OWNER_PAIRS[index][1],
        **frames,
        **section,
        ...
        "end_mm": float(end),
        "run_mm": float(span),
        "path_mm": float(np.sum(lengths)),
        "contour_mm": float(contour.length),
        "contour_fraction": float(span / contour.length),
        "wavelength_mm": float(wavelength),
        "periods": int(periods),
        "depth_scale": float(scale),
        "max_groove_depth_mm": float(np.max(section["groove_depth_mm"])),
        "min_groove_depth_mm": float(np.min(section["groove_depth_mm"])),
        "max_cut_depth_mm": float(np.max(section["cut_depth_mm"])),
        "min_local_depth_mm": float(np.min(section["local_depth_mm"])),
        "min_rear_land_mm": rear_land,
        "min_front_land_mm": float(np.min(front_land)),
        "min_wall_after_cut_mm": wall,
        "volume_mm3": float(np.sum(0.5 * (area[:-1] + area[1:]) * lengths)),
        "fed_through": "rear_seam_gap",
    }


def dovetail_wedge_volume_mm3(layout, index: int, local_depth) -> float:
    """Adhesive volume the depth-varying relief alone opens on one joint.

    Integrated, not booleaned: the relief is a linear wedge in the depth
    direction, truncated by the local rear surface, so its volume closes in
    one integral along the joint contour.  Booleaning it out of a graded wing
    would cost minutes and OCC would then integrate the result to only about
    half a percent, which is coarser than the whole answer.
    """
    contour = dovetail_joint_contour(layout, index)
    stations = np.arange(0.0, contour.length + 1.0e-9, 0.05)
    points = [contour.interpolate(float(value)) for value in stations]
    coords = np.array([[point.x, point.y] for point in points], dtype=float)
    depth = np.asarray(local_depth(coords), dtype=float)
    plan = np.maximum(dovetail_plan_clearance_mm(
        np.minimum(stations, contour.length - stations)), 0.0)
    # Integral of the depth scale from the local rear surface to the front.
    apex = DOVETAIL_TAPER_APEX_OFFSET_MM
    through = (DOVETAIL_FRONT_CLEARANCE_MM * depth
               + (DOVETAIL_REAR_CLEARANCE_MM - DOVETAIL_FRONT_CLEARANCE_MM)
               * (0.5 * depth**2 + apex * depth) / DEPTH_MM
               ) / DOVETAIL_REAR_CLEARANCE_MM
    return float(np.trapezoid(plan * through, stations))


def _channel_section_area(groove_depth, height):
    """Female material one channel section removes, per unit length.

    Measured from the female flank, the section is a rectangle
    ``groove_depth`` deep by ``height`` tall with its rear-facing corner cut
    back on the 45-degree ramp the front-down print has to bridge.
    """
    groove_depth = np.asarray(groove_depth, dtype=float)
    height = np.asarray(height, dtype=float)
    return groove_depth * (height - groove_depth) + 0.5 * groove_depth**2


def _partition(field: Polygon):
    # The source-derived split keeps all three front-down prints within the
    # 220-mm bed.  Each lower print owns one male trapezoid as a constant
    # prism; the next-higher print owns the depth-tapered female complement.
    y_ref = 198.5
    slope = -0.08
    ...
    # ``wings`` cuts every piece from the nominal partition and then sweeps
    # the depth-varying relief into the female half, so it needs the
    # two-piece nominal masks as well as the rear-face plan ones above.
    two_piece_nominal = _largest_polygon(
        unary_union((lm_upper, um)).buffer(0))
    if two_piece_nominal.geom_type != "Polygon":
        raise RuntimeError(
            "two-piece LM/UM nominal mask is not one connected polygon")
    two_piece_nominal_parts = {
        "lm_lower": lm_lower,
        "lm_um_upper": two_piece_nominal,
    }
    if any(part.is_empty for part in (*nominal_parts.values(),
                                      *print_parts.values(),
                                      *two_piece_nominal_parts.values(),
...
def _build_layout(definition: VariantDefinition) -> VariantLayout:
    ...
    (nominal_parts, print_parts, two_piece_print_parts,
     two_piece_nominal_parts, lower_seam_y, lower_seam_slope, joint_seams,
    ...
    layout = VariantLayout(definition, profile, field, nominal_parts,
                           print_parts, two_piece_print_parts,
                           two_piece_nominal_parts, structure,
...
def _validate_layout(layout: VariantLayout) -> None:
    ...
    for index, (gap, seam) in enumerate(zip(
        ...
        # The one-sided slit closes short of both field endpoints; assert the
        # prescribed full rear clearance is reached after the taper.
        length = seam.length
        if length <= 2.0 * DOVETAIL_ENDPOINT_TAPER_MM:
            raise RuntimeError(
                f"{layout.definition.key} seam {index} cannot taper clearance")
        full_width = float(dovetail_plan_clearance_mm(
            DOVETAIL_ENDPOINT_TAPER_MM))
        if not math.isclose(full_width, DOVETAIL_REAR_CLEARANCE_MM,
                            abs_tol=1e-12):
            raise RuntimeError("dovetail clearance smootherstep drifted")
        dry_land = dovetail_endpoint_dry_land_mm()
        if not 0.2 <= dry_land <= 0.5 * DOVETAIL_ENDPOINT_TAPER_MM:
            raise RuntimeError(
                f"dovetail endpoint dry land {dry_land:.4f} mm is out of "
                "range")
    sites = _right_sites()
    for name in ACTIVE_RECEIVER_NAMES_RIGHT:
        site = sites[name]
...
def _draw_depth_and_table(
    ...
    ax.text(ox + ow / 2.0, oy + oh + 1.5,
            "bonded male/female dovetail — depth unchanged",
            fontsize=6.4, ha="center", va="bottom")
    seam_contract = (
        f"female clearance {DOVETAIL_FRONT_CLEARANCE_MM:.2f} front to "
        f"{DOVETAIL_REAR_CLEARANCE_MM:.2f} rear; "
        f"{DOVETAIL_ENDPOINT_TAPER_MM:.1f} endpoint closure; "
        "zero envelope growth\n"
    ...
    ax.text(4.0, 49.0,
            ...
            "Z-axis insertion from the rear; dovetails register XY.\n"
            "Z retention is cured epoxy in the depth-tapered gallery and its "
            "serpentine channel, not friction: assemble\n"
            "dry, clamp flush, then inject along the rear seam gap, which the "
            "taper holds open at 0.20 mm the whole\n"
            "length of the joint and which feeds the channel through its own "
            "flank. Both joints and the shared\n"
            "one-layer knife-tip endpoint require proof tests.",
            fontsize=5.55, va="top", color="#30383f")
