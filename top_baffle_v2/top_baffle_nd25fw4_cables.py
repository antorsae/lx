"""Cable routing profiles for the proud family and skeletal V1LF.
...
``LX_ROUTING_PROFILE=v1lf`` (R6F) selects the isolated two-carrier
...
``top_baffle_nd25fw4_v1lf_route`` with continuous 0.8 mm minimum skins and
...
no detachable external raceway or printed V1LF grommet.
...
the UM route. The proud R14 handoff and complete V1LF physical centerlines,
including their intentional free spans, are checked by ``test_clearances.py``
and ``test_v1lf_r6f.py`` using eroded-outline containment and full-centerline
...
if ROUTING_PROFILE not in {"proud", "v1lf"}:
    raise ValueError(
        "LX_ROUTING_PROFILE must be 'proud' or 'v1lf', got "
        f"{ROUTING_PROFILE!r}"
    )
ROUTING_REV = {"proud": "R6P", "v1lf": "R6F"}[ROUTING_PROFILE]
...
def route_points(name, um_handoff_key: str = "proud"):
    ...
    if name == "um":
        if ROUTING_PROFILE != "proud":
            raise RuntimeError(
                "V1LF UM routing is integral core geometry; use "
                "top_baffle_nd25fw4_v1lf_route.route_cable_points()"
            )
...
def um_handoff_points(n: int = 24, um_handoff_key: str = "proud"):
    ...
    if ROUTING_PROFILE != "proud":
        raise RuntimeError(
            "V1LF has covered local Z bumps, not an R14 outlet handoff"
        )
    spec = _um_handoff_spec(um_handoff_key)
    sx, sy, sz = spec["start"]
...
def route_centerline_points(name: str, spacing_mm: float = 2.0,
    ...
    """
    if name == "um" and ROUTING_PROFILE == "v1lf":
        if um_handoff_key != "proud":
            raise RuntimeError(
                "um_handoff_key applies to proud-family routes, not V1LF"
            )
        if raceway_only:
            raise RuntimeError("V1LF has no detachable raceway")
        from top_baffle_nd25fw4_v1lf_route import route_cable_points
    ...
    if raceway_only:
        raise RuntimeError("raceway_only was removed with V1LF's raceway")
...
def _tube_loft(points, radii, sides: int = 24):
    ...
    loft has no internal cap and also supports the V1LF dipped tunnel
    cutters.
    """
...
def um_tube(raceway_only: bool = False, radial_extra_mm: float = 0.0,
    ...
    if ROUTING_PROFILE != "proud" or raceway_only or radial_extra_mm:
        raise RuntimeError("external V1LF raceway generation was removed")
...
def cable_cutters(um_handoff_key: str = "proud", route_names=None,
    ...
    if ROUTING_PROFILE != "proud":
        raise RuntimeError(
            "V1LF routing is integral to its two carrier cores; "
