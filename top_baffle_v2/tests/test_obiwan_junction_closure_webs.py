...
ROOT = PROJECT_ROOT
CORE_SOURCE = ROOT / "src/lx521_baffle/obiwan/carriers.py"
CLOSURE_SOURCE = ROOT / "src/lx521_baffle/obiwan/closure_webs.py"
JOINT_SOURCE = ROOT / "src/lx521_baffle/obiwan/joints.py"
MAGNET_SOURCE = ROOT / "src/lx521_baffle/obiwan/magnets.py"
ADDON_SOURCE = ROOT / "src/lx521_baffle/obiwan/attachments.py"
...
def test_source_owns_full_depth_webs_before_functional_recuts() -> None:
    ...
    closure = CLOSURE_SOURCE.read_text(encoding="utf-8")
    ...
    assert "_printable_lens_components" in closure
    ...
    lm_plan_source = closure[
        closure.index("def lm_um_closure_polygons"):
        closure.index("def _t_crescent_boundary_y")]
    t_plan_source = closure[
        closure.index("def t_um_closure_polygons"):
        closure.index("def junction_closure_polygons")]
...
def obiwan_actual_parts(stand_foot: bool | None = None):
    ...
    manifest = ROOT / "build" / state / ".obiwan_stage/manifest.json"
