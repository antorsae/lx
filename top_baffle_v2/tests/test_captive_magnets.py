...
import lx521_baffle.magnet_contract as contract
import lx521_baffle.magnets as captive
...
def test_nominal_authority() -> None:
    assert captive.DEFAULT_SPEC is contract.DEFAULT_SPEC
    assert captive.CaptiveMagnetSpec is contract.CaptiveMagnetSpec
    assert (captive.CaptiveMagnetGeometryError
            is contract.CaptiveMagnetGeometryError)
    ...
    # value: 0.52 + 2.10 + 0.52 must serialize as exactly 3.14.
    assert facts["captive_land_mm"] == 3.14
    assert captive.CAPTIVE_LAND_MM == 3.14
    assert facts["paired_magnet_face_separation_mm"] == 1.09
    assert captive.NOMINAL_PAIRED_FACE_SEPARATION_MM == 1.09
...
def test_coupon_style_wall_solids() -> None:
    ...
    # spacing allowance must remain solid in front of the qualified 0.52-mm
    ...
    receiver = Pos(0.0, -4.0, 0.0) * Box(
        4.0, 8.0, 11.5, align=(Align.MIN, Align.MIN, Align.MIN))
    # Source z=8.0 keeps the complete deeper 0.52-skin land inside the
    # 11.5-mm coupon blocks on both the roof/inner-skin side (print top
    # 9.32) and the chimney side (land ends 2.18 above the local rear).
    kwargs = {
        "name": "coupon_lm",
        "face": (0.0, 0.0, 8.0),
        "outward": (1.0, 0.0, 0.0),
    ...
    assert base.is_inside((-0.26, 0.0, 8.0), tolerance=1.0e-5)
    assert not base.is_inside(
        base_tools.cavity_center_xyz, tolerance=1.0e-5)
    assert base.is_inside((-2.88, 0.0, 8.0), tolerance=1.0e-5)
    assert len(receiver_tools.cutters) == 3
    assert receiver.is_inside((0.025, 0.0, 8.0), tolerance=1.0e-5)
    assert receiver.is_inside((0.31, 0.0, 8.0), tolerance=1.0e-5)
    assert receiver.is_inside((0.545, 0.0, 8.0), tolerance=1.0e-5)
    assert not receiver.is_inside((0.595, 0.0, 8.0), tolerance=1.0e-5)
    assert not receiver.is_inside(
        receiver_tools.cavity_center_xyz, tolerance=1.0e-5)
    assert receiver.is_inside((2.93, 0.0, 8.0), tolerance=1.0e-5)
...
def test_axis_parallel_and_front_down_opposed_cones() -> None:
    ...
    assert math.isclose(tools.roof_start_print_z_mm, 2.8, abs_tol=1e-12)
    assert math.isclose(
        tools.required_min_part_top_print_z_mm, 5.92, abs_tol=1e-12)
    assert block.is_inside((0.0, 0.0, 0.26), tolerance=1.0e-5)
    ...
    assert all(math.isclose(a, b, abs_tol=1.0e-12) for a, b in zip(
        axial.cavity_center_xyz, (0.0, 0.0, 4.17)))
    assert all(math.isclose(a, b, abs_tol=1.0e-12) for a, b in zip(
        axial.seated_magnet_center_xyz, (0.0, 0.0, 4.17)))
    assert math.isclose(axial.roof_start_print_z_mm, 15.18, abs_tol=1e-12)
    assert math.isclose(axial.roof_apex_print_z_mm, 17.78, abs_tol=1e-12)
    assert math.isclose(
