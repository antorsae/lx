...
def _require_guarded_worker() -> None:
    ...
    if not memory_guard.is_guarded_process():
        raise RuntimeError("Obi-Wan stage worker escaped the active memory guard")
...
def _worker_count(output: Path) -> None:
    from top_baffle_nd25fw4_obiwan_route import route_inner_cutter_group_count
...
def _worker_lm_outer(output: Path) -> None:
    from top_baffle_nd25fw4_obiwan import lm_carrier_outer_blank
...
def _worker_lm_cut(input_path: Path, output: Path, index: int) -> None:
    ...
    from top_baffle_nd25fw4_obiwan import apply_lm_route_cutter
...
def _worker_lm_finalize(input_path: Path, output: Path) -> None:
    from build123d import import_brep
    from top_baffle_nd25fw4_obiwan import finalize_lm_carrier
...
def _worker_lm_direct(output_dir: Path) -> None:
    ...
    from top_baffle_nd25fw4_obiwan import lm_carrier
    from top_baffle_nd25fw4_obiwan_lm_split import lm_carrier_split_parts
...
def _worker_print_group(group: str, output_dir: Path) -> None:
    if group == "um":
        from top_baffle_nd25fw4_obiwan import um_carrier
        parts = {"core_um_carrier": um_carrier()}
    elif group == "tweeter":
        from top_baffle_nd25fw4_obiwan_attachments import tweeter_crescent
...
def _worker_review_group(group: str, output_dir: Path) -> None:
    if group == "static":
        ...
        from top_baffle_nd25fw4_um_fit import obiwan_lm_cable_envelope
        parts = {"reference_lm_cable": obiwan_lm_cable_envelope()}
    elif group == "ts_cable":
        from top_baffle_nd25fw4_um_fit import obiwan_ts_cable_envelope
