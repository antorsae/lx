"""Final V1LF R6F Z-first core tunnels plus free rear cable spans.
...
def _require_guarded_build():
    ...
    if not memory_guard.is_guarded_process():
        raise RuntimeError(
            "V1LF route geometry must run through run_memory_guarded.py")
...
# independently rechecked below and in ``test_v1lf_r6f.py``.
...
if _CROSS_GEOM.geom_type != "Point":
    raise RuntimeError(f"V1LF crown routes must cross once, got {_CROSS_GEOM}")
...
def required_handoff_shell_components(route_name, owner_filter=None):
    """V1LF route exits are native flush mouths with no printed handoff."""
    _require_guarded_build()
