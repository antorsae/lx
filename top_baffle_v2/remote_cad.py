...
def _source_paths(*, include_candidate_outputs: bool = False) -> tuple[Path, ...]:
    ...
    for path in BAFFLE_DIR.rglob("*"):
        ...
        if relative.parts and relative.parts[0] in STATE_OUTPUT_ROOTS:
...
def _transport_binding_from_manifest(manifest: dict) -> dict:
    ...
    matches = [
        ...
        and record.get("path") == TRANSPORT_SOURCE_PATH
    ]
    if len(matches) != 1:
        raise RuntimeError(
    ...
    return {
        "source_path": TRANSPORT_SOURCE_PATH,
...
def _artifact_paths(work: Path) -> list[Path]:
    ...
    for name in ARTIFACT_SCAN_ROOTS:
        root = baffle / name
        if root.is_dir():
            paths.update(
    ...
    try:
        ...
        with build_lock_path.open("a+b") as build_lock, \
            ...
            for name in (
                ...
                "LX_CLEARANCE_SINGLE_CHECK", "LX_R6F_SINGLE_CHECK",
            ):
                env.pop(name, None)
            cache = job / "cache"
            temporary = job / "tmp"
...
def _job_executor_path(metadata: dict) -> str:
    ...
    if (not isinstance(transport, dict)
            ...
            or transport.get("source_path") != TRANSPORT_SOURCE_PATH
            or not re.fullmatch(
...
def _promote_artifacts(incoming: Path, metadata: dict) -> int:
    ...
    for root_name in sorted(full_roots):
        source = incoming_root / root_name
    ...
    for source in sorted(incoming.rglob("*")):
        ...
        if (len(relative.parts) >= 2 and relative.parts[0] == "top_baffle_v2"
                and relative.parts[1] in full_roots):
            continue
    ...
    if targets == ["clean"]:
        ...
        destinations = [
            (BAFFLE_DIR / root_name,
             (BAFFLE_DIR / root_name).relative_to(REPO_ROOT), None)
            for root_name in sorted(full_roots)
        ]
    ...
    try:
        if targets != ["clean"]:
            for root_name in sorted(full_roots):
                source = incoming_root / root_name
                destination = BAFFLE_DIR / root_name
                destination.parent.mkdir(parents=True, exist_ok=True)
...
def _run_guarded_local_promotion(
    ...
    command = [
        str(checker_python), str(BAFFLE_DIR / "run_memory_guarded.py"), "--",
...
def _wait_and_fetch(remote: Remote, metadata: dict) -> int:
    ...
    try:
        ...
        print(
            ...
            f"python3 remote_cad.py resume {metadata['job_id']}",
            file=sys.stderr,
        )
        return 130
...
def _run_remote(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="lx-cad-source-") as temporary_text:
        ...
        _launch_remote_job(remote, metadata)
    if args.detach:
        print(f"Detached. Resume with: python3 remote_cad.py resume {job_id}")
        return 0
