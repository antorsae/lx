...
Public commands::
    ...
    python export_v1lf_staged.py stage --manifest STATE/.v1lf_stage/manifest.json
    python export_v1lf_staged.py step --manifest ... --kind split --output ...
...
def _validate_guard_policy_record(policy: object) -> dict:
    ...
    if not isinstance(policy, dict) or set(policy) != expected_keys:
        raise RuntimeError("V1LF stage guard-policy record is malformed")
    profile_name = policy["memory_profile"]
    if profile_name not in memory_guard.MEMORY_PROFILES:
        raise RuntimeError("V1LF stage memory-profile record is unknown")
...
if __name__ == "__main__":
    ...
    if not memory_guard.is_guarded_process():
        guard = SCRIPT.with_name("run_memory_guarded.py")
...
PRINT_PART_SPECS = {
    "core_lm_carrier": {
        ...
        "stl_name": "lx521_top_v1lf_core_1of2_lm_carrier",
        "group": "lm",
    },
    "core_um_carrier": {
    ...
    "addon_tweeter_crescent": {
        ...
        "stl_name": "lx521_top_v1lf_addon_tweeter_crescent",
        "group": "tweeter",
    },
}
...
REVIEW_PART_SPECS = {
    ...
    "reference_y_bundle": {
        ...
        "label": "REFERENCE_v1lf_Y_breakout_bundle_heatshrink",
        "group": "um_service",
    },
    "reference_y_terminal_lead_1": {
        "filename": "review_reference_y_terminal_lead_1.brep",
        "label": (
            "REFERENCE_v1lf_Y_breakout_terminal_lead_1_heatshrink"),
        "group": "um_service",
    },
    "reference_y_terminal_lead_2": {
        "filename": "review_reference_y_terminal_lead_2.brep",
        "label": (
            "REFERENCE_v1lf_Y_breakout_terminal_lead_2_heatshrink"),
        "group": "um_service",
    },
    "reference_lm_cable": {
...
def _require_v1lf_profile() -> None:
    profile = os.environ.get("LX_ROUTING_PROFILE", "v1lf")
    if profile != "v1lf":
        raise RuntimeError(
            "V1LF staging requires LX_ROUTING_PROFILE=v1lf")
...
def _source_fingerprint(
    ...
    digest = hashlib.sha256()
    digest.update(
        f"lx521-v1lf-release-native-brep-stage-v{SCHEMA_VERSION}\0"
        .encode("ascii"))
    digest.update(b"\0floor=" + (b"1" if stand_foot else b"0"))
    digest.update(b"\0profile=v1lf")
    ...
    paths = [
        SCRIPT,
        ROOT / "run_memory_guarded.py",
        *sorted(ROOT.glob("top_baffle_nd25fw4*.py")),
        *REFERENCE_INPUTS,
    ]
    for path in paths:
        if not path.is_file():
            raise FileNotFoundError(
                f"required V1LF stage input is missing: {path}")
        try:
...
def load_stage_manifest(
    ...
    if stand_foot is None:
        ...
        _require_v1lf_profile()
    else:
        stand_foot = bool(stand_foot)
    expected_state = _state_name(stand_foot)
    if payload.get("schema_version") != SCHEMA_VERSION:
        raise RuntimeError(f"unsupported V1LF stage manifest: {path}")
    if payload.get("state") != expected_state:
        raise RuntimeError(
            f"V1LF stage state {payload.get('state')!r} != {expected_state!r}")
    if payload.get("stand_foot") is not stand_foot:
        raise RuntimeError("V1LF stage stand-foot flag mismatch")
    if payload.get("routing_profile") != "v1lf":
        raise RuntimeError("V1LF stage routing-profile mismatch")
    if payload.get("lm_cutter_group_count") != EXPECTED_LM_CUTTER_GROUP_COUNT:
        raise RuntimeError("V1LF stage cutter-group count mismatch")
    ...
    if (not isinstance(runtime_identity, dict)
        ...
        raise RuntimeError("V1LF stage runtime-identity record is malformed")
    ...
    if payload.get("source_sha256") != expected_source:
        raise RuntimeError("V1LF stage is stale for the current CAD sources")
    if require_active_environment:
        if guard_policy != _guard_policy():
            raise RuntimeError("V1LF stage guard-policy mismatch")
        if runtime_identity != _runtime_identity():
            raise RuntimeError("V1LF stage runtime-identity mismatch")
    ...
    for section, expected_specs in expected_sections.items():
        ...
        for key, record in records.items():
            ...
            if (record["path"] != spec["filename"]
                ...
                raise RuntimeError(
                    f"V1LF stage identity record mismatch: {key}")
...
def _require_guarded_worker() -> None:
    ...
    if not memory_guard.is_guarded_process():
        raise RuntimeError("V1LF stage worker escaped the active memory guard")
...
def _worker_count(output: Path) -> None:
    from top_baffle_nd25fw4_v1lf_route import route_inner_cutter_group_count
...
def _worker_lm_outer(output: Path) -> None:
    from top_baffle_nd25fw4_v1lf import lm_carrier_outer_blank
...
def _worker_lm_cut(input_path: Path, output: Path, index: int) -> None:
    ...
    from top_baffle_nd25fw4_v1lf import apply_lm_route_cutter
...
def _worker_lm_finalize(input_path: Path, output: Path) -> None:
    from build123d import import_brep
    from top_baffle_nd25fw4_v1lf import finalize_lm_carrier
...
def _worker_print_group(group: str, output_dir: Path) -> None:
    if group == "um":
        ...
        raise RuntimeError(f"unknown V1LF print group: {group}")
    ...
    if set(parts) != expected:
        raise RuntimeError(
            f"V1LF {group} part set {sorted(parts)} != {sorted(expected)}")
    for key, shape in parts.items():
        _export_brep_transaction(
            shape, output_dir / PRINT_PART_SPECS[key]["filename"],
            require_single_solid=True)
...
def _worker_review_group(group: str, output_dir: Path) -> None:
    if group == "static":
        ...
        y_parts = v1lf_y_breakout_boot_parts()
        parts = {
            "reference_um_cable": rear_cable_envelope("v1lf"),
            "reference_y_bundle": (
                y_parts["v1lf_Y_breakout_bundle_heatshrink"]),
            "reference_y_terminal_lead_1": (
                y_parts["v1lf_Y_breakout_terminal_lead_1_heatshrink"]),
            "reference_y_terminal_lead_2": (
                y_parts["v1lf_Y_breakout_terminal_lead_2_heatshrink"]),
        }
    elif group == "lm_cable":
        from top_baffle_nd25fw4_um_fit import v1lf_lm_cable_envelope
        parts = {"reference_lm_cable": v1lf_lm_cable_envelope()}
    elif group == "ts_cable":
        from top_baffle_nd25fw4_um_fit import v1lf_ts_cable_envelope
    ...
    if set(parts) != expected:
        raise RuntimeError(
            f"V1LF {group} review set {sorted(parts)} != {sorted(expected)}")
    for key, shape in parts.items():
        _export_brep_transaction(
...
def _worker_step(manifest_path: Path, kind: str, output: Path) -> None:
    ...
    if kind == "split":
        ...
        root_label = (
            "lx521_v1lf_r6f_assembled_floor" if stand_foot else
            "lx521_v1lf_r6f_assembled_no_floor_fused_solid_web")
    else:
        raise RuntimeError(f"unknown V1LF STEP kind: {kind}")
    ...
    try:
        ...
        from export_steps import _validate_step_transaction
        _validate_step_transaction(temporary)
        temporary.replace(output)
    finally:
        temporary.unlink(missing_ok=True)
    print(f"[v1lf-stage] wrote {kind} STEP {output}", flush=True)
...
def _stage(manifest_path: Path) -> None:
    ...
    if manifest_path.is_file():
        try:
            ...
            if payload["source_sha256"] == source_sha256:
                print(f"[v1lf-stage] reused {manifest_path}", flush=True)
                return
    ...
    with tempfile.TemporaryDirectory(
        ...
        if count != EXPECTED_LM_CUTTER_GROUP_COUNT:
            raise RuntimeError(
                f"V1LF exporter expects {EXPECTED_LM_CUTTER_GROUP_COUNT} "
    ...
    print(f"[v1lf-stage] wrote {manifest_path}", flush=True)
