...
def prepare(keys=None):
    ...
    assert all(x >= config['minimum_purge_mm3']
...
def preserved_check():
    """Preserve this lane's GF baseline while allowing recorded later GF work.

    Subsequent authorized changes to GF do not invalidate independent
    translucent G-code. Every such revision must have explicit before/after
    hashes and a passing report; unexplained changes still fail.
    """
    old=json.loads((WORK/'preserved_GF_files.json').read_text())
    expected=old.copy();applied=[]
    revisions=WORK/'authorized_GF_revisions.json'
    for revision in json.loads(revisions.read_text()) if revisions.exists() else []:
        report_path=ROOT/revision['report']
        assert sha(report_path)==revision['report_sha256']
        report=json.loads(report_path.read_text());assert report['status']=='pass'
        for path,change in revision['files'].items():
            assert expected[path]==change['before_sha256'],path
            expected[path]=change['sha256']
        applied.append(revision['report'])
    assert all((ROOT/p).is_file() and sha(ROOT/p)==digest for p,digest in expected.items()), 'Unrecorded GF output change'
    return dict(files_preserved_at_translucent_creation=len(old),
                baseline_files_still_identical=sum(expected[p]==v for p,v in old.items()),
                later_authorized_GF_revisions=applied)
...
def publish():
    ...
    manifest=dict(status='sliced_static_checks_passed',physical_print_test='not performed with these materials',
        ...
        GF_preservation=preserved_check(),source_files={rel(p):sha(p) for p in files},projects=rows)
...
def verify():
    ...
    print('Verified',len(manifest['projects']),'translucent jobs; GF history:',preserved_check(),flush=True)
