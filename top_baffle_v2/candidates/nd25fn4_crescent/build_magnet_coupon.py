...
def main():
    ...
    report=dict(status='pass',physical_qualification='pending',policy_sha256=policy_sha256(),
                ...
                changeover=audit_changeover(toolpath),pauses=rows, walls=[])
    for target,authority,position in cropped:
...
''')
    ...
    with (output/'README.md').open('a') as stream:
        stream.write(f"\nChangeover calibration: **{materials['purge_each_direction_mm3']:g} mm³ purge in both directions**, **{materials['interface_flush_volumetric_speed_mm3_s']:g} mm³/s PLA flush**. The new slice is approximately **{minutes} minutes** including native setup and changes. Printing temperatures are unchanged. Observe both material-change directions for continuous extrusion, residual material, clicking or loading errors; record the outcome before using this calibration for a full body.\n")
