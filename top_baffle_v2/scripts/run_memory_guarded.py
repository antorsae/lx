...
def require_guarded_build(message: str) -> None:
    """Fail with the caller's established message outside a live guard."""
    if not is_guarded_process():
        raise RuntimeError(message)


def reexec_under_guard(
    script: str | os.PathLike[str],
    argv: list[str] | tuple[str, ...] | None = None,
) -> None:
    """Re-run an unguarded script through this module and propagate its exit.

    A guarded child returns immediately, preventing recursive wrapping.  The
    default argument vector is the current CLI tail exactly as received.
    """
