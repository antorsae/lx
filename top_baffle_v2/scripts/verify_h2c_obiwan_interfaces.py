#!/usr/bin/env python3
"""Check the continuous H2C LM against the retained UM and Dayton ND25FN-4 interfaces."""
...
def main():
    ...
    dayton=solid(body)^crop;body_crop=to_trimesh(dayton)
    ...
    for state in ('no_floor_stand','floor_stand'):
        ...
        overlap=abs((dayton^lm).volume());regular_overlap=abs((regular^lm).volume())
        ...
        um_front=dayton^md.Manifold.cube((120,40,30)).translate((-60,300,18.299))
        ...
        rows.append(dict(state=state,LM=lm_source,Dayton_ND25FN4_overlap_mm3=overlap,regular_UM_overlap_mm3=regular_overlap,
            receivers=receivers,Dayton_ND25FN4_LM_front_occlusion_mm2=occlusion,seam_samples=samples))
    write_json(ROOT/'build/h2c/obiwan_interface_validation.json',dict(status='pass',Dayton_ND25FN4=body_source,regular_UM=regular_source,
        source_sha256=sha256_file(Path(__file__)),checks=rows,physical_fit='hardware trial pending'))
    print('H2C full LM: regular UM and Dayton ND25FN-4 clear; Dayton faces flush and LM uncovered in both stand states.',flush=True)
