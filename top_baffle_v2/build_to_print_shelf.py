...
``slice_captive_magnets.py``.  The nine non-magnet pieces are sliced locally
...
from front_down_contract import FrontDownContractError, validate_print_sidecar
...
def _sha256(path: Path) -> str:
    return captive.sha256_file(path)
...
def _sha256_bytes(data: bytes) -> str:
    return captive._sha256_bytes(data)
