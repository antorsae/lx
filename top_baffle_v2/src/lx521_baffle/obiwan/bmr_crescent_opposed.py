...
``proud.vase_tebm35c10_4``: two D63 lands side by side on the vase's own
...
everything that touches the collar: the same half-lap interface, the same
preserved acoustic axis, the same flush junction skirt, the same hidden Ø6.00
mate-face entry
and its stadium collar.  Both sets live in ``bmr_pod``.

The two lands
-------------
The lower land uses the same preserved acoustic axis as the coaxial pod.  The
upper land sits one vase pitch above it.  The two D63 cores overlap by 13.7 mm,
so the body is one continuous plan with a 39.224 mm waist between the driver
axes, and both lands carry the vase's two magnet flats.  The optional BMR-slim
topology keeps a D56 core, a 6.7 mm overlap and the same magnet-face width.
...
--------------------
All four of the vase's captive D5 x 2 side stations, two per driver land, at the
...
# --- the two lands -----------------------------------------------------------
# The lower land uses the preserved coaxial-pod axis.  The upper land is one
# vase pitch
...
def topology_land_overlap_mm(
    topology: str | BmrLandTopology = FULL_CIRCULAR_LAND,
) -> float:
    land_spec = resolve_land_topology(topology)
    return round(2.0 * land_spec.core_r_mm - AXIS_PITCH_MM, 9)


def topology_waist_half_width_mm(
    topology: str | BmrLandTopology = FULL_CIRCULAR_LAND,
) -> float:
    land_spec = resolve_land_topology(topology)
    if land_spec.core_r_mm <= AXIS_PITCH_MM / 2.0:
        return 0.0
    return round(math.sqrt(
        land_spec.core_r_mm ** 2 - (AXIS_PITCH_MM / 2.0) ** 2), 9)
...
def land_radius_at_z(
    z: float,
    topology: str | BmrLandTopology = FULL_CIRCULAR_LAND,
) -> float:
    """Land core radius at one Z; its plan is constant through the stack."""
...
def _pod_solid(
    ...
    """Both selected lands over the common 25.1-mm envelope."""
    return _fuse_required(
        land_solid(LOWER_AXIS_XY[1], *POD_Z_SPAN, topology),
        land_solid(UPPER_AXIS_XY[1], *POD_Z_SPAN, topology),
        "upper BMR land onto the lower")
...
def build_model(
    topology: str | BmrLandTopology = FULL_CIRCULAR_LAND,
) -> BmrCrescentOpposedModel:
    if topology_waist_half_width_mm(topology) <= INTER_POCKET_BRANCH_R_MM:
        raise RuntimeError("selected BMR lands leave no side cover on branch")
    part, magnet_tools = apply_land_magnets(
        bmr_crescent_opposed_body(topology), MAGNET_LANDS, topology)
    solids = list(part.solids())
...
def declared_openings(
    ...
    block instead.
    """
    branch_side_cover = round(
        topology_waist_half_width_mm(topology)
        - INTER_POCKET_BRANCH_R_MM,
        9,
    )
    return [
        {
        ...
        cable_entry_opening(),
        {
            ...
            "side_cover_mm": branch_side_cover,
            "count": 1,
        },
        {
...
def design_facts(
    ...
    land_spec = resolve_land_topology(topology)
    waist_half_width = topology_waist_half_width_mm(topology)
    branch_side_cover = round(
        waist_half_width - INTER_POCKET_BRANCH_R_MM, 9)
    return {
        "part": PART_NAME,
        ...
        "silhouette": {
            ...
            "minimalism_rule": (
                ...
                "plan is the two selected lands, the two bosses, the fill "
        ...
        "pod": {
            **land_facts(topology),
            "lands": 2,
            "land_overlap_mm": topology_land_overlap_mm(topology),
            "waist_half_width_mm": waist_half_width,
            "waist_y_mm": WAIST_Y_MM,
            "waist_note": (
                f"the two {land_spec.core_d_mm:g} mm cores overlap by "
                f"{topology_land_overlap_mm(topology):g} mm, so the body "
                "is one continuous plan and its core waist between the "
                f"driver axes is {2.0 * waist_half_width:g} mm"),
        },
        ...
        "cable": {
            **cable_entry_facts(topology),
            ...
            "branch_side_cover_mm": branch_side_cover,
            "branch_rule": (
        ...
        "declared_openings": declared_openings(topology),
        "exterior_openings": [
            opening["name"] for opening in declared_openings(topology)
