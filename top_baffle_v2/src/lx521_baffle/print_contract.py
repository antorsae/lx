...
from .io import pretty_json_bytes, sha256_file
...
def front_down_transform_record(
    oriented_bbox_min_mm: Sequence[Any],
    *,
    z_rotation_deg: Any = 0.0,
) -> dict[str, Any]:
    """Construct canonical X180-plus-Z metadata from an oriented bbox.

    The caller still owns CAD orientation, bed-fit selection, and placement.
    ``oriented_bbox_min_mm`` is the minimum after X180 and the selected in-bed
    Z rotation but before translation to the STL origin.
    """
    minimum = _vec3(
        oriented_bbox_min_mm, "oriented_bbox_min_mm")
    z_angle = _finite_number(z_rotation_deg, "z_rotation_deg")
    translation = tuple(-value for value in minimum)
    ...
    record: dict[str, Any] = {
        ...
        "rotation_deg": {"x": 180.0, "z": z_angle},
        "pre_translation_bbox_min_mm": list(minimum),
        "stl_origin_translation_mm": list(translation),
    }
    validate_front_down_transform(record, label="constructed print transform")
    return record
...
def _validate_print_sidecar_payload(
    ...
    *,
    ...
    if digest != sha256_file(stl_path):
        raise FrontDownContractError(
...
def write_print_sidecar(
    ...
    *,
    ...
    try:
        serialized = pretty_json_bytes(payload, allow_nan=True)
