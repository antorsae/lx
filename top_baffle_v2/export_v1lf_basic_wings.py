#!/usr/bin/env python3
"""Transactional STEP/STL/review exporter for one V1LF Ac/Ae wing family.

The geometry authority is :mod:`v1lf_basic_wings_cad`.  This module owns only
...
CONTRACT_SOURCE = SCRIPT_DIR / "gen_v1lf_basic_variants.py"
NO_FLOOR_STAGE_MANIFEST = (
    SCRIPT_DIR / "no_floor_stand/.v1lf_stage/manifest.json")
...
INTERFACE_SOURCES = (
    ...
    SCRIPT_DIR / "top_baffle_nd25fw4_v1lf.py",
    SCRIPT_DIR / "top_baffle_nd25fw4_v1lf_attachments.py",
    SCRIPT_DIR / "top_baffle_nd25fw4_a_comp.py",
    SCRIPT_DIR / "top_baffle_nd25fw4_v1lf_bridge.py",
    SCRIPT_DIR / "top_baffle_nd25fw4_v1lf_floor.py",
    SCRIPT_DIR / "top_baffle_nd25fw4_v1lf_floor_strength.py",
    SCRIPT_DIR / "top_baffle_nd25fw4_v1lf_lm_split.py",
    SCRIPT_DIR / "top_baffle_nd25fw4_v1lf_route.py",
    NO_FLOOR_STAGE_MANIFEST,
...
CANONICAL_STEP_TEMPLATE = "top_baffle_nd25fw4_v1lf_wing_{slug}.step"
ASSEMBLED_STEP_TEMPLATE = (
    "top_baffle_nd25fw4_v1lf_wing_{slug}_assembled.step")
FACTS_TEMPLATE = "v1lf_wing_{slug}_facts.json"
MANIFEST_TEMPLATE = "v1lf_wing_{slug}_print_manifest.json"
REVIEW_KINDS = (
...
def _part_label(slug: str, side: str, order: int, role: str) -> str:
    return f"v1lf_wing_{slug}_{side}_{order}of3_{role}"


def _stl_name(slug: str, side: str, order: int, role: str) -> str:
    return f"lx521_top_v1lf_wing_{slug}_{side}_{order}of3_{role}.stl"
...
def _context_mesh_records(parts: dict[str, dict]) -> list[dict[str, Any]]:
    ...
    identities = {
        "lm_lower_floor": {
            "label": "V1LF LM lower — floor stand",
            "color": FLOOR_CONTEXT_COLOR,
            "linestyle": FLOOR_CONTEXT_LINESTYLE,
            "linewidth": STATE_CONTEXT_LINEWIDTH,
            "style_name": "dotted",
            "legend_label": (
                "LM lower — floor stand (green dotted; common outline)"),
            "side_main": False,
        },
        "lm_lower_no_floor": {
            "label": "V1LF LM lower — no-floor",
            "color": NO_FLOOR_CONTEXT_COLOR,
            "linestyle": NO_FLOOR_CONTEXT_LINESTYLE,
            "linewidth": STATE_CONTEXT_LINEWIDTH,
            "style_name": "dash_dot",
            "legend_label": (
                "LM lower — no-floor (blue dash-dot; common outline)"),
            "side_main": True,
        },
        "lm_upper": {
            "label": "V1LF LM upper — no-floor reference",
            "color": CONTEXT_COLOR,
            "linestyle": CONTEXT_LINESTYLE,
            "linewidth": CONTEXT_LINEWIDTH,
            "style_name": "dotted_neutral",
            "legend_label": "LM upper / UM / T — no-floor reference",
            "side_main": True,
        },
        "um": {
            "label": "V1LF UM — no-floor reference",
            "color": CONTEXT_COLOR,
            "linestyle": CONTEXT_LINESTYLE,
            "linewidth": CONTEXT_LINEWIDTH,
            "style_name": "dotted_neutral",
        ...
        "t": {
            "label": "V1LF T crescent — no-floor reference",
            "color": CONTEXT_COLOR,
            "linestyle": CONTEXT_LINESTYLE,
            "linewidth": CONTEXT_LINEWIDTH,
            "style_name": "dotted_neutral",
    ...
    if set(parts) != set(identities):
        raise RuntimeError(
            "V1LF review context must contain two LM-lower states plus "
            "LM-upper, UM and T")
    records = []
    for key, style in identities.items():
        ...
        indices = np.asarray(triangles, dtype=int)
        if xyz.size == 0 or indices.size == 0:
            raise RuntimeError(f"empty V1LF review context mesh: {key}")
        records.append({
            "key": key,
            **style,
...
def _draw_mesh_review(
    ...
    fig.savefig(
        ...
        metadata={"Title": title,
                  "Description": (
                      "Project-native V1LF Ac/Ae CAD QA render with dual-"
...
def _draw_side_section_review(
    ...
    fig.savefig(
        ...
        metadata={"Title": title,
                  "Description": (
                      "True world-X section of V1LF Ac/Ae CAD with dual-"
                      "state V1LF LM-lower silhouettes: coincident common "
                      "profile, blue dash-dot no-floor and green dotted "
                      "floor stand")})
    plt.close(fig)
...
def _render_reviews(
    ...
    outputs = {kind: review_dir / f"v1lf_wing_{slug}_{kind}.png"
               for kind in REVIEW_KINDS}
    _draw_mesh_review(
        outputs["front"], records,
        title=(f"V1LF {slug.upper()} — acoustic front / common LM-lower "
               "profile"),
        elev=90.0, azim=-90.0, hide_z_axis=True,
        context_records=context_records)
    _draw_mesh_review(
        outputs["rear"], records,
        title=f"V1LF {slug.upper()} — rear surface / dovetail seams",
        elev=-90.0, azim=90.0, hide_z_axis=True,
        context_records=context_records)
    _draw_side_section_review(
        outputs["side_section"], records,
        title=(f"V1LF {slug.upper()} — true right-wing rear-depth profile "
               "(Y/Z section)"),
        context_records=context_records)
    _draw_mesh_review(
        outputs["split_exploded"], records,
        title=f"V1LF {slug.upper()} — six-piece exploded print assembly",
        elev=28.0, azim=-62.0, exploded=True,
        context_records=context_records)
    _draw_magnet_root_review(
        outputs["magnet_roots"], records,
        title=f"V1LF {slug.upper()} — right LM/UM magnetic roots",
...
def _export_variant(slug: str, output_root_arg: Path) -> dict[str, Any]:
    os.environ.setdefault("LX_ROUTING_PROFILE", "v1lf")
    ...
    try:
        ...
        for side in SIDES:
            ...
            shape.label = f"v1lf_wing_{slug}_{side}_monolithic"
        ...
        canonical_label = f"lx521_v1lf_basic_wing_{slug}_monolithic_pair"
        ...
        assembly_label = f"lx521_v1lf_basic_wing_{slug}_print_assembly"
        assembled_children = [
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Export one complete V1LF Ac/Ae wing artifact family")
    parser.add_argument(
